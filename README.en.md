...
> [!TIP]
> Documentation examples use `visionpower@latest` so you always get the latest fixes. For strictly reproducible environments (CI, shared hosts), pin `latest` to an exact version and review the [CHANGELOG](./CHANGELOG.md) before upgrading.
...
> [!WARNING]
> In the current version, an empty `allowedDirs` means **absolute paths are unrestricted**. If an agent can construct a path, VisionPower may read any supported image readable by the current OS user. Production and shared environments should always configure a minimal directory allowlist.
...
Install and launch in one step (idempotent, safe to re-run):

```bash
npx -y visionpower@latest setup-dsh --launch
```

The installer pins the dsh plugin to the installer's own exact version (the version `npx visionpower@latest` actually resolved this run), so the plugin never drifts with `latest` inside dsh. If `pnpm` is missing, it stops with guidance and does not modify the global package manager. The high-privilege `setup_visionpower` administration tool is not registered by default; an operator must explicitly set `enableAdminTool: true`. The dsh tool reads attachments only from the most recent image-bearing user message in the current session; it does not scan recent files or guess across sessions.

See [`src/dsh/README.md`](./src/dsh/README.md) for full instructions. After a dsh upgrade (rc.6 → rc.7 → rc.8 …), rerun the installer to verify and reapply the patch; do not assume an old patch remains safe or applicable.
